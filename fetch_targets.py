...
CATEGORIES = {
    ...
    "refinery": ("#ffd23f", [
        'nwr["man_made"="petroleum_well"]', 'nwr["industry"="oil"]',
        'nwr["man_made"="works"]["product"~"oil|petroleum|fuel",i]',
        'nwr["man_made"="works"]["name"~"нефт|НПЗ|нефтепереработ",i]']),
    "fuel_depot": ("#ffa03f", [
...
def classify(t):
    ...
    if ind == "chemical" or any(w in name for w in ("химич", "порох", "взрывч", "боеприпас")):
        return "chemical"
    if ind == "oil" or mm == "petroleum_well" or \
       any(w in name for w in ("нефтеперераб", "нпз", "oil", "refinery")):
        return "refinery"
