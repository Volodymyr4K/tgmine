...
class TestText(unittest.TestCase):
    ...
    def test_hit_is_a_consequence_not_an_intent(self):
        ...
        # побажання — не звіт
        for t in ("Русня має горіти.", "Рф должна гореть от Шмякино до Владивостока.",
                  "Площа досить велика. Мав би горіти 2+ дня.", "коли почнуть горіти ваші «Пятьорочкі»",
                  "Нехай горить"):
    ...
    def test_denials(self):
        for t in ("зафіксованих об'єктивним контролем влучань не було",
                  ...
                  "Твердо и четко. Разрушеній нєт, здоровю нє угрожаєт."):
...
class TestPlaces(unittest.TestCase):
    ...
    def test_more_region_adjective_forms(self):
        # множина й знахідний перед «регіони/область» — регіон, а не селище
        self.assertEqual(self.names("Уральські регіони тривожаться, обломок долетів."), [])
        self.assertEqual(self.names("Вночі була масована атака на Ростовську область."), [])
        self.assertEqual(self.names("Атакували Курську обл., пожежа."), [])
        # перед «районом» — будь-який прикметник
        self.assertEqual(self.names("Пожар в Железнодорожном районе после атаки."), [])
        # орудний іменника — місто: «під Нижньокамськом, Республіка Татарстан»
        self.assertEqual(self.names("Пожар под Нижнекамском, Республика Татарстан."), ["Nizhnekamsk"])
        self.assertEqual(self.names("В Краснодарском крае была атакована компрессорная станция."), [])
        # «обломки» — не «обл.»
        sks = [A.skel(w) for w in ("курские", "обломки")]
        self.assertFalse(A._before_region(sks, 0))
        self.assertEqual(self.names("Коломна, обломки."), ["Kolomna"])
    ...
    def test_future_does_not_make_an_event(self):
        # «Склады будут гореть» — погроза, не подія: місце-підпис не стає ударом
        a = A.analyse({"id": 1, "date": "2026-09-16T22:33:00+00:00", "text": "Воронеж. / Склады будут гореть, и никакое прикрытие им "
                                                   "не поможет, вот увидите, всё ещё впереди, ждите новостей."},
                      self.P, self.ST.point_region, (), self.ST.depth_km)
        self.assertEqual([x["anchored"] for x in a["places"]], [False])
...
class TestIncidents(unittest.TestCase):
    ...
    def test_blown_up_is_an_event(self):
        # 20760 (17.05 08:17): здогад «щось підірвали» тримає ніч удару, а
        # підтвердження о 12:19 — до нього, не на наступну ніч
        ps = [post(1, "2026-05-17 08:17", "Велика ймовірність що СОУ знову щось підірвали в Каспійську. "
                                          "Можливо під вогневе ураження потрапили кораблі."),
              post(2, "2026-05-17 12:19", "Є підтвердження, обʼєктивний контроль ураження сторожового корабля в Каспійську.")]
        self.assertEqual([(x["night"], x["n"]) for x in self.build(ps)], [("2026-05-16", 2)])
    ...
    def test_continuing_fire_is_a_detail(self):
        # «продолжает гореть» без відомого удару — не новий удар (пополудні
        # відкривав би інцидент не на ту ніч, рецензія 24.09)
        self.assertEqual(self.build([post(1, "2026-08-10 14:00", "В Нижнекамске продолжает гореть НПЗ после атаки.")]), [])
        # руїни пополудні без відомого удару — теж ні
        self.assertEqual(self.build([post(1, "2026-08-04 15:00", "Все что осталось от склада ВБ в Алексине после атаки.")]), [])
    ...
    def test_series_rules_from_review(self):
        # наслідок в іншому місці без інциденту рве серію; продовження не
        # подовжують серію й не рухають `last`; чужа область — не продовження
        ps = [post(1, "2026-08-16 04:00", "Самара, горит НПЗ после атаки."),
              post(2, "2026-08-16 04:10", "Пишут о прилете в Харькове, пожар."),
              post(3, "2026-08-16 04:20", "Горят 2 объекта.")]
        self.assertEqual([x["n"] for x in self.build(ps)], [1])
        ps = [post(1, "2026-09-10 04:00", "Самара, горит НПЗ после атаки."),
              post(2, "2026-09-10 13:30", "Самара, на НПЗ пожар."),
              post(3, "2026-09-10 14:10", "Горит."),
              post(4, "2026-09-11 00:00", "Самара, прилет по НПЗ, пожар.")]
        self.assertEqual(sorted((x["night"], x["n"]) for x in self.build(ps)),
                         [("2026-09-09", 3), ("2026-09-10", 1)])
        ps = [post(1, "2026-08-10 04:00", "Воронеж, горит склад после атаки."),
              post(2, "2026-08-10 04:20", "А в башкирии тоже горит склад.")]
        self.assertEqual([x["n"] for x in self.build(ps)], [1])
        # ланцюжок «Горит.» кожні 40 хв: лише перше — продовження
        ps = [post(1, "2026-08-10 09:00", "Воронеж, горит склад после атаки."),
              post(2, "2026-08-10 09:40", "Горит."),
              post(3, "2026-08-10 10:20", "Горит."),
              post(4, "2026-08-10 11:00", "Горит.")]
        self.assertEqual([x["n"] for x in self.build(ps)], [2])
    ...
    def test_placeless_post_continues_the_series(self):
        ps = [post(1, "2026-07-06 14:05", "Омськ, горить найбільший в рф НПЗ після атаки."),
              ...
              # невпізнана назва без області — теж не продовження
              post(6, "2026-07-06 14:36", "Горить склад у Кукуєвці."),
              # довгий пост без місця — не продовження
              post(4, "2026-07-06 14:38", "Горить. " + "дуже багато слів про щось інше, не про серію взагалі. " * 3),
              # поза вікном серії
    ...
    def test_abbreviation_is_not_a_sentence_end(self):
        # «у респ. Башкортостан» — назва, а не початок речення
        self.assertTrue(A._names_something("А це логістичний хаб Wildberries у респ. Башкортостан."))
        self.assertFalse(A._names_something("Горить. Розгорається."))
        # велика літера з нового рядка — теж початок речення
        self.assertFalse(A._names_something("Розгорається\nМасштаб великий."))
