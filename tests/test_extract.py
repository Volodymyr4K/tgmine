...
class TestSubjectsAddedSept23(unittest.TestCase):
    """17 субʼєктів, яких конфіг не знав (23 вересня 2026), і звуження, які
    знайшов А/Б на всьому вікні. Кожен рядок — пастка, що вже спрацювала або
    спрацювала б; тексти дослівні з `data/` (переноси рядків — як у пості).
    """
    ...
    def test_new_subjects_are_known(self):
        for text, want in [
                ("Республика Марий Эл\nОпасность по БПЛА", "Марій Ел"),
                ("Республика Чувашия\nОпасность по БПЛА", "Чувашія"),
                ("Чувашская Республика - опасность по БПЛА.", "Чувашія"),
                ("Кировская область\nОтбой опасности по БПЛА", "Кіровська"),
                ("Череповец\nВологодская область\nФиксация БПЛА", "Вологодська"),
                ("Республика Калмыкия\nОпасность по БПЛА", "Калмикія"),
                ("Псковская область\nВнимание по БПЛА", "Псковська"),
                ("Карачаево-Черкесская Республика\nОпасность по БПЛА", "Карачаєво-Черкесія"),
                ("Грозный, Чеченская Республика - угроза атаки БПЛА.", "Чечня"),
                ("Республика Коми\nОпасность по БПЛА", "Комі")]:
            with self.subTest(want=want):
                self.assertEqual(self.regions(text)[:1], [want])

    def test_petersburg_only_as_first_word(self):
        """Першим словом — Ленінградська. Після району — ні: місто на 5 млн
        ставало city-marker і забирало крапку в району (64 крапки на 24–33 км)."""
        self.assertEqual(self.regions("Санкт-Петербург\nОпасность по БПЛА"),
                         ["Ленінградська"])
        self.assertEqual(self.regions("Кронштадт\nСанкт-Петербург\nРабота ПВО по БПЛА"), [])

    def test_baltic_primorsk_is_not_zaporizhzhia(self):
        """884 пости про Приморськ — запорізькі, 24 — балтійські (їхали за 1600 км)."""
        self.assertNotIn("ТОТ_Запоріжжя", self.regions(
            "Кингисепп, Усть-Луга, Выборг, Приморск и близлежащие, Ленинградская "
            "область - опасность по БПЛА."))
        self.assertIn("ТОТ_Запоріжжя", self.regions(
            "Приморск\nБердянск и близлежащие\nОпасность по БПЛА"))

    def test_lookalikes_stay_where_they_are(self):
        for text, bad in [
                # село Ульяновської, а не Чувашія
                ("Чувашский Сайман, Николаевский район, Ульяновская область - "
                 "пролёт БПЛА.", "Чувашія"),
                # місто Кіров у Калузькій
                ("Жиздра, Людиново, Киров, Калужская область - опасность по БПЛА",
                 "Кіровська"),
                # район Карачаєво-Черкесії
                ("Адыге-Хабльский район, Карачаево-Черкесская Республика - группа "
                 "БПЛА повернула на Невинномысск", "Адигея"),
                # lpr1 так пише Сєвєродонецьк
                ("От  Кременной в сторону Северодвинска фиксации БПЛА", "Архангельська"),
                # з помилкою в назві розвʼязувалось у Великий Новгород
                ("Усвяты, Поречье, Великий Луки и близлежащие населённые пункты", "Псковська"),
                # «Майкоп» у переліку Кубані перебивав Краснодар
                ("Краснодар\nМайкоп и близлежащие\nТревога по БПЛА", "Адигея")]:
            with self.subTest(bad=bad, text=text[:25]):
                self.assertNotIn(bad, self.regions(text))
