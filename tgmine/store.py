...
# «От X» / «со стороны X» ДЛЯ ОБЛАСТІ — це звідки летять, не місце. Для
# села правило протилежне й лишається (див. вище): «От Белой Березки»
# описує спостереження біля неї. Область же в такому реченні завжди чужа:
# «Тобольск, Тюмень, Тюменская область - внимание по БПЛА от Свердловской
# области» ставило крапку в Єкатеринбурзі (маркер області, 1.5 млн) замість
# Тобольська. Заміряно на архіві: правило разом із «г.» вище міняє 550
# подій із 194 тис., точок далі 250 км від власної області стає менше
# (3882 -> 3859), у вибірці з 25 хибних не знайдено. Загальний пріоритет
# села над маркером міста натомість відкинуто: 14 533 змін і «далеких»
# більше на 2271 — маркер міста надійніша координата за село-тезку.
SRC_BEFORE = re.compile(
    r"(?:\bот|со\s+стороны)\s+(?:г\.\s*|города\s+)?$", re.I)
...
def point_entity(p):
    ...
    aim = {id(e) for e in pts if DIR_BEFORE.search(before(e))
           or (e.get("type") == "регіон" and SRC_BEFORE.search(before(e)))}
