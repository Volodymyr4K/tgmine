...
def geocode_vectors(posts: list[dict], gaz, region_geo: dict,
    ...
    конфіга, і тут вони ЗАБОРОНА, не пошук: імʼя, яке є областю («Брянская»),
    у районний фолбек не йде — інакше «Брянская район» знаходило міський
    округ Брянська. Без обох фолбек мовчить, і поведінка та сама, що була.
    ...
    «Хомутовский»), який лежить у газетирі як «<назва> район»; ще частина —
    артефакти розбору («Продолжаются», «Над Белгородской»). Кожен
    відновлений вектор — це ланка маршруту `declared` замість `inferred`.

    Область як кінець вектора НЕ відновлюється свідомо. Була спроба віддавати
    центроїд для «от Брянской области» — і вона зробила два лиха: кінці з
    українськими ключами («Брянська») не впізнавав фільтр областей у
    `routes.py` (він знає лише « oblast», « krai»…), і 9 центроїдів за шість
    ночей потрапили в маршрути редактора — рівно те, від чого стоїть
    `TestAreaEndpoints`; а міста-маркери з патернів регіонів («Казань»,
    «Москву») давали центроїд ЗАМІСТЬ міста, прикриваючи провал пошуку.
    Карта нальоту векторів не малює, маршрути області відкидають — тобто в
    центроїда-кінця нема жодного законного споживача.
    """
    out = []
    for p in posts:
        vs = parse(p["text"])
        ...
        for v in vs:
            a = _first_hit(v["src"], gaz, near, max_km, a1, region_rx)
            b = _first_hit(v["dst"], gaz, near, max_km, a1, region_rx)
            if not b:
                continue
            out.append({
                "t": p["date"], "url": p["url"],
...
def _is_region(n, region_rx):
    """Імʼя — область (або її маркер) за патернами конфіга. Область — не район."""
    return bool(region_rx) and any(rx.search(n) for rx in region_rx.values())


def _first_hit(names, gaz, near, max_km, a1=None, region_rx=None):
    # Спершу звичайний пошук по ВСІХ іменах, і лише коли не розвʼязалось
    # жодне — фолбек. Не навпаки: перша версія застосовувала фолбек до
    ...
    for n in names:
        ...
        # зі своєї області, інакше це інший здогад. Район — це 40 км, і
        # `routes.py` такий кінець приймає (див. TestAreaEndpoints).
        if a1 and _ADJ.search(n) and not _is_region(n, region_rx):
